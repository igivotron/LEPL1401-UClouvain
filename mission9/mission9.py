...
class ArticleReparation(Article):
    def __init__(self,h):
        """
        Initialise la classe ArticleReparation, une sous-classe de Article
        :param h: un float, le nombre d'heure de travail
        """
        self.__duree = h

    def description(self):
        """
        Cette méthode retourne une desciption du service (réparation) et du temps consommé.
        :return: Retourne une indication de réparation suivi du temps consommé
        """
        return "Reparation ({d})".format(d=self.__duree)
...
class Piece:
    def __init__(self, d, pu, m=None, frag=False, tva_red=False):
        self.__description = d
        self.__prix_unitaire = pu
        self.__masse = m
        self.__fragile = frag
...
class ArticlePiece(Article):
    ...
    def description(self):
        return "{a} * {b} @ {c}".format(a=self.__nombre, b=self.__piece.description(), c=self.__piece.prix())


    def prix(self):
        return self.__nombre * self.__piece.prix()
